...
out_file = root_dir.joinpath("files/complaint.xml")

def booklet_to_xml():
    root = ET.Element('xml')
    for i in _booklet:
        complaint_tag = ET.SubElement(root, 'complaint')
        ET.SubElement(complaint_tag, 'id').text = i['id']
        ET.SubElement(complaint_tag, 'name').text = i['name']
        ET.SubElement(complaint_tag, 'text').text = i['text']
...
def get_main_menu_choice():
    ...
    m_menu_entries = {
        ...
        4: {"text": "To XML.", "f": booklet_to_xml},
