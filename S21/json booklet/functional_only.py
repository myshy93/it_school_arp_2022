...
out_file = root_dir.joinpath("files/complaint.json")

def load_from_json():
    global _booklet
    try:
        with open(out_file) as fin:
            _booklet = json.load(fin)
    except Exception as err:
        print(err)
    else:
        print("Loaded complaints from JSON.")
...
def get_main_menu_choice():
    ...
    m_menu_entries = {
        ...
        4: {"text": "Export to JSON.", "f": booklet_to_json},
